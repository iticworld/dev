...
static void red_black_tree_print_insertion_duplicate(int value)
{
    return;
#ifdef    __EMSCRIPTEN__
    EM_ASM_INT({
...
static void red_black_tree_print_insertion(int value, const char * format, ...)
{
    char s[1024];
    va_list ap;
    va_start(ap, format);
...
static void red_black_tree_print_insertion_fixup(const char * format, ...)
{
    char s[1024];
    va_list ap;
    va_start(ap, format);
...
static void red_black_tree_print_deletion_fixup(const char * format, ...)
{
    char s[1024];
    va_list ap;
    va_start(ap, format);
...
static void red_black_tree_print_deletion(int value, const char * format, ...)
{
    char s[1024];
    va_list ap;
    va_start(ap, format);
...
struct node * red_black_tree_add(struct tree * t, int value)
{
    ...
    {
        t->size += 1;
        red_black_tree_print_insertion(value, "ROOT = NEW(%d, BLACK)", value);
    ...
    while(n != NULL)
    {
        if(n->value == value)
        {
            red_black_tree_print_insertion_duplicate(value);
            return n;
        }
        else if(n->value < value)
        {
            ...
            {
                t->size += 1;
                red_black_tree_print_insertion(value, "- %c(%d)'S RIGHT = NEW(%d, RED)", n->color == 1 ? 'R' : 'B' ,n->value, value);
        ...
        else
        {
            ...
            {
                t->size += 1;
                red_black_tree_print_insertion(value, "- %c(%d)'S LEFT = NEW(%d, RED)", n->color == 1 ? 'R' : 'B' ,n->value, value);
...
extern void red_black_tree_fixup_add(struct tree * t, struct node * n)
{
    ...
    while(n->parent != NULL && n->parent->color == RED)
    {
        ...
        if(grandparent != NULL)
        {
            ...
            {
                red_black_tree_print_insertion_fixup("- FIXUP[RECOLOR]: %c(%d) AND UNCLE == RED", n->color == 1 ? 'R' : 'B' ,n->value);
            ...
            if(n == parent->left)
            {
                ...
                {
                    red_black_tree_print_insertion_fixup("- FIXUP[TRIANGLE]: %c(%d) == BLACK AND UNCLE == RED", n->color == 1 ? 'R' : 'B' ,n->value);
                    n = parent;
            ...
            if(n == parent->left)
            {
                if(grandparent->left == parent)
                {
                    red_black_tree_print_insertion_fixup("- FIXUP[LINE]: %c(%d) == BLACK AND UNCLE == RED", n->color == 1 ? 'R' : 'B' ,n->value);
                    grandparent->color = RED;
                    parent->color = BLACK;
    ...
    if(t->root->color == RED)
    {
        red_black_tree_print_insertion_fixup("- FIXUP[ROOT]: %c(%d) == RED", t->root->color == 1 ? 'R' : 'B' , t->root->value);
    }
    t->root->color = BLACK;
}
...
struct node * red_black_tree_del(struct tree * t, int value)
{
    ...
    if(n != NULL)
    {
        red_black_tree_print_deletion(value, "- DELETE[BST] %c(%d)", n->color==1 ? 'R' : 'B', n->value);
        if(n->left == NULL)
        {
            if(n->parent)
            {
            ...
            if(n->color == BLACK)
            {
                if(red_black_tree_node_color(n->right)==BLACK)
                {
        ...
        else if(n->right == NULL)
        {
            ...
            if(n->color == BLACK)
            {
                if(red_black_tree_node_color(n->left)==BLACK)
                {
                    red_black_tree_fixup_del(t, n->parent);
                }
                else
        ...
        else
        {
            ...
            if(color == BLACK)
            {
                if(red_black_tree_node_color(right) == RED)
                {
                    right->color = BLACK;
                }
                else
                {
                    red_black_tree_fixup_del(t, parent != n ? parent : min);
                }
            }
        }
    }
...
static void red_black_tree_change_color(struct node * x, struct node * y)
{
    int color = x->color;
    x->color = y->color;
    y->color = color;
...
void red_black_tree_fixup_del(struct tree * t, struct node * parent)
{
    ...
    while(red_black_tree_node_color(n)==BLACK && parent != NULL)
    {
        ...
        if(n != sibling)
        {
            if(red_black_tree_node_color(sibling) == RED)
            {
                /** case 1 sibling is red */
                red_black_tree_print_deletion_fixup("- FIXUP/RIGHT[COLOR(SIBLING)=RED] R(%d)", sibling->value);
                red_black_tree_print_deletion_fixup(" - CHANGE COLOR (PARENT, SIBLING)");
                red_black_tree_print_deletion_fixup(" - ROTATE(LEFT, PARENT)");
                red_black_tree_print_deletion_fixup(" - UPDATE(SIBLING)");
                red_black_tree_change_color(parent, sibling);
            ...
            if(red_black_tree_node_color(sibling->right)==BLACK)
            {
                ...
                {
                    sibling->color = RED;
                    if(red_black_tree_node_color(parent)==BLACK)
                    {
                        red_black_tree_print_deletion_fixup("- FIXUP/RIGHT[COLOR(SIBLING)=BLACK(BLACK, BLACK), COLOR(PARENT)=BLACK] B(%d) - B(%d)", parent->value, sibling->value);
                        red_black_tree_print_deletion_fixup(" - SET COLOR (SIBLING, RED)");
                        red_black_tree_print_deletion_fixup(" - UPDATE(N, PARENT)");
                        red_black_tree_print_deletion_fixup(" - UPDATE(PARENT, N'S PARENT)");
                        red_black_tree_print_deletion_fixup(" - REBALANCE");
                        n = parent;
                        parent = n->parent;
                        continue;
                    }
                    ...
                    {
                        red_black_tree_print_deletion_fixup("- FIXUP/RIGHT[COLOR(SIBLING)=BLACK(BLACK, BLACK), COLOR(PARENT)=BLACK] R(%d) - B(%d)", parent->value, sibling->value);
                        red_black_tree_print_deletion_fixup(" - CHANGE COLOR (PARENT, SIBLING)");
                        red_black_tree_print_deletion_fixup(" - DONE");
                        parent->color = BLACK;
                        break;
                    }
                }
                red_black_tree_print_deletion_fixup("- FIXUP/RIGHT[COLOR(SIBLING)=BLACK(RED, BLACK)] - B(%d)", sibling->value);
                red_black_tree_print_deletion_fixup(" - SET COLOR(SIBLING, RED)");
                red_black_tree_print_deletion_fixup(" - SET COLOR(SIBLING's LEFT, BLACK)");
                red_black_tree_print_deletion_fixup(" - ROTATE(RIGHT, SIBLING)");
                red_black_tree_print_deletion_fixup(" - UPDATE(SIBLING, PARENT'S RIGHT)");
                sibling->color = RED;
                sibling->left->color = BLACK;
                red_black_tree_rotate_right(t, sibling);
                sibling = parent->right;
            }
            red_black_tree_print_deletion_fixup("- FIXUP/RIGHT[COLOR(SIBLING)=BLACK(UNKNOWN, RED)] - B(%d)", sibling->value);
            red_black_tree_print_deletion_fixup(" - SET COLOR(SIBLING, PARENT'S COLOR)");
            red_black_tree_print_deletion_fixup(" - SET COLOR(PARENT, BLACK)");
            red_black_tree_print_deletion_fixup(" - SET COLOR(SIBLING'S RIGHT, BLACK)");
            red_black_tree_print_deletion_fixup(" - ROTATE(LEFT, PARENT)");
        ...
        else
        {
            ...
            {
                /** case 1 sibling is red */
                red_black_tree_print_deletion_fixup("- FIXUP/LEFT[COLOR(SIBLING)=RED] R(%d)", sibling->value);
                red_black_tree_print_deletion_fixup(" - CHANGE COLOR (PARENT, SIBLING)");
                red_black_tree_print_deletion_fixup(" - ROTATE(RIGHT, PARENT)");
                red_black_tree_print_deletion_fixup(" - UPDATE(SIBLING)");
                red_black_tree_change_color(parent, sibling);
            ...
            if(red_black_tree_node_color(sibling->left)==BLACK)
            {
                if(red_black_tree_node_color(sibling->right)==BLACK)
                {
                    ...
                    if(red_black_tree_node_color(parent)==BLACK)
                    {
                        red_black_tree_print_deletion_fixup("- FIXUP/LEFT[COLOR(SIBLING)=BLACK(BLACK, BLACK), COLOR(PARENT)=BLACK] B(%d) - B(%d)", parent->value, sibling->value);
                        red_black_tree_print_deletion_fixup(" - SET COLOR (SIBLING, RED)");
                        red_black_tree_print_deletion_fixup(" - UPDATE(N, PARENT)");
                        red_black_tree_print_deletion_fixup(" - UPDATE(PARENT, N'S PARENT)");
                        red_black_tree_print_deletion_fixup(" - REBALANCE");
                    ...
                    {
                        red_black_tree_print_deletion_fixup("- FIXUP/LEFT[COLOR(SIBLING)=BLACK(BLACK, BLACK), COLOR(PARENT)=BLACK] R(%d) - B(%d)", parent->value, sibling->value);
                        red_black_tree_print_deletion_fixup(" - CHANGE COLOR (PARENT, SIBLING)");
                        red_black_tree_print_deletion_fixup(" - DONE");
                        parent->color = BLACK;
                        break;
                ...
                red_black_tree_print_deletion_fixup("- FIXUP/LEFT[COLOR(SIBLING)=BLACK(BLACK, RED)] - B(%d)", sibling->value);
                red_black_tree_print_deletion_fixup(" - SET COLOR(SIBLING, RED)");
                red_black_tree_print_deletion_fixup(" - SET COLOR(SIBLING's RIGHT, BLACK)");
                red_black_tree_print_deletion_fixup(" - ROTATE(LEFT, SIBLING)");
                red_black_tree_print_deletion_fixup(" - UPDATE(SIBLING, PARENT'S LEFT)");
                sibling->color = RED;
                sibling->right->color = BLACK;
                red_black_tree_rotate_left(t, sibling);
                sibling = parent->left;
            }
            red_black_tree_print_deletion_fixup("- FIXUP/LEFT[COLOR(SIBLING)=BLACK(UNKNOWN, RED)] - B(%d)", sibling->value);
            red_black_tree_print_deletion_fixup(" - SET COLOR(SIBLING, PARENT'S COLOR)");
            red_black_tree_print_deletion_fixup(" - SET COLOR(PARENT, BLACK)");
            red_black_tree_print_deletion_fixup(" - SET COLOR(SIBLING'S LEFT, BLACK)");
            red_black_tree_print_deletion_fixup(" - ROTATE(RIGHT, PARENT)");
